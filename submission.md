...
Evidence: Instrumentation output:
...
Root Cause: The parse_records() function has three silent failure points:

fread(&records[i].type, 1, 1, fp) != 1 breaks without logging
fread(&records[i].length, 2, 1, fp) return value not even checked
if (!records[i].payload) break breaks without logging Only the initial calloc failure is logged with sec_warn. All other failures are completely silent.
Impact:
...
Evidence: Instrumentation output with truncated input:
...
Root Cause: The code at lines 44-45 in main.c:
...
Evidence: Instrumentation output with 6 records:
...
Root Cause: Lines 7-9 in parser.c:
...
Evidence: Test with 1-byte input file:
...
#endif
Trigger Condition: This bug requires specific conditions rarely seen in normal testing:

Environment variable DATAPROC_FAST=1 must be set (enables FAST_MODE)
Input file must have at least one valid record
The payload buffer was allocated exactly rec->length bytes in parse_records()
The memcpy reads rec->length + 1 bytes, exceeding the allocation by 1
...
Root Cause: Line 14 in memory.c:
